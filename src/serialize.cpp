...
// Data size is 4 + 128 + 4 + 1 = 137 bytes. We pad to 140 bytes as this is the nearest divisible by 4 we have. So we add 3 bytes
...
TResult deserialize(const char *buffer, int len, void *output) {
	...
	if(result == PACKET_COMPLETE) {
		...
		if(comms->magic != MAGIC_NUMBER) {
			printf("BAD MAGIC NUMBER. EXPECTED %lu GOT %lu\n", MAGIC_NUMBER, comms->magic);
			return PACKET_BAD;
		}
		...
		if(checksum != comms->checksum) { return PACKET_CHECKSUM_BAD; }
		else {
