...
serverKeyPath = "tls/alex.key"
serverCertPath = "tls/alex.crt"
caCertPath = "tls/signing.pem"
expectedClientName = "yours.com"

# Buffer and message Properties
NET_TYPE_SIZE = struct.calcsize("=B") 	# standard unsigned C char, maps to a python int
CMD_SIZE = struct.calcsize("=I")		# standard unsigned C int_32, maps to python int
...
def handleNetworkData(buffer:bytes):
	...
	packetType = struct.unpack("=B", buffer[0:NET_TYPE_SIZE])[0]
...
def handleCommand(buffer:bytes):
	...
	Args:
		message (str): The message received from the TLS connection.
	"""
	# command1 = struct.unpack("=c", buffer[0:CMD_SIZE])[0]
	command1 = struct.unpack("=I", buffer[0:CMD_SIZE])[0]
