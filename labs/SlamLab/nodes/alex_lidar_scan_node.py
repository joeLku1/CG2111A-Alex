...
def lidarScanThread(setupBarrier:Barrier=None, readyBarrier:Barrier=None):
    ...
    This function has been heavily modified. Instead of publishing the raw LIDAR data to the "lidar/scan" topic,
    start a TCP websocket on port 8765, and send the data there instead. This allows another, more powerful computer to
    access the LIDAR data and build the SLAM map there, freeing up valuable RPi resources.
    """
    # Setup
    ...
    for i in range(attempts):
        try:
            ...
            if all([x is not None for x in [health, scan_mode_info, sr]]):
                break
    ...
    # Start TCP server
    client_socket = start_server('0.0.0.0', 8765)
    ...
    try:
        ...
        for count, scan in enumerate(scan_generator()):
            ...
            if results and current_round["r"] > INITIAL_ROUNDS_IGNORED:
                ...
                # Default code, publishes LIDAR data to the lidar/scan topic
                # publish(LIDAR_SCAN_TOPIC, results)

                # Modification: sending over TCP websocket instead of publishing

                # Send data over TCP
                # Data is converted into JSON format before sending, and decoded before reading on the other system
                try:
                    results_json = json.dumps(results).encode('utf-8') + b'\n'
                    client_socket.sendall(results_json)
...
def start_server(host, port):
    """
    Start TCP server to send data
    Args:
        host: 0.0.0.0 by default, thus accessible via the RPi's local IPv4 address
        port: 8765 by default, an unused port
