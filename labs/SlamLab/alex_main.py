...
from nodes.data_node import dataThread
...
ENABLE_ARDUINO_INTERFACE = False
...
def main():
    ...
    with PubSubManager() as mgr:
        ...
        dataNode = 1
        setupbarrier_data_t = Barrier(dataNode + 1) # 1 thread + 1 main thread.
        readybarrier_data_t = Barrier(dataNode + 1) # 1 thread + 1 main thread.
        mgr.add_thread(target=dataThread,
                name="Data Thread",
                kwargs={"setupBarrier": setupbarrier_data_t, "readyBarrier": readybarrier_data_t})
        print("Barriers set up")
        ...
        setupbarrier_data_t.wait() # Wait for data thread to be ready before proceeding
        readybarrier_data_t.wait() # Wait for data thread to be ready before proceeding
